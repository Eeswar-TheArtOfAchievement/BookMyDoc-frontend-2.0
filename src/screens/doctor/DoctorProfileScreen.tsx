...
const DoctorProfileScreen = ({navigation}) => {
  ...
  const [tempDetails, setTempDetails] = useState(doctorDetails);
  const [image, setImage] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [isFocused, setIsFocused] = useState(false);
  useEffect(() => {
    setTempDetails(doctorDetails);
  }, [doctorDetails]);
    ...
    <View style={styles.container}>
      <View style={styles.card}>
        <View style={styles.photoContainer}>
          {doctorDetails.profileImage ? (
            <View style={styles.imageContainer}>
              <Image
                source={{
                  uri: `data:image/jpeg;base64,${doctorDetails.profileImage}`,
                }}
                style={styles.image}
              />
        ...
        <View style={styles.infoContainer}>
          {[
            {icon: 'user', label: 'Full Name', value: doctorDetails.fullName},
            {icon: 'envelope', label: 'Email', value: doctorDetails.email},
            {
              icon: 'phone',
              label: 'Phone',
      ...
      <TouchableOpacity
        ...
        <Text style={styles.linkText}>Hii</Text>
      </TouchableOpacity>
      <TouchableOpacity
        style={styles.link}
